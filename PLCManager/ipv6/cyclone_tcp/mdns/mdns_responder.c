   ...
   do
   {
      ...
      if(interface->numServices > 0 && interface->instanceName != NULL)
      {
         ...
         for(j = 0; j < interface->numServices; j++)
         {
            //Format PTR resource record
            error = dnsSdAddPtrResourceRecord(interface,
               &interface->services[j], TRUE, message, &length);
            //Any error to report?
            if(error) break;
         }
      }
   ...
   DnsHeader *message, size_t *length, bool_t flush)
{
   ...
   //Copy IPv4 address
   ipv4CopyAddr(resourceRecord->rdata, &interface->ipv4Context.addr);
   ...
   DnsHeader *message, size_t *length, bool_t flush)
{
   ...
   //Copy IPv6 address
   ipv6CopyAddr(resourceRecord->rdata, &interface->ipv6Context.addrList[0].addr);
