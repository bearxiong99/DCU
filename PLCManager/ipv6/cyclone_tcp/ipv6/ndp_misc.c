         ...
         if(interface->linkState)
         {
            ...
            else
            {
               ...
               if(timeCompare(time, entry->timestamp + entry->dadTimeout) >= 0)
               {
                  ...
                  else if(entry->dadRetransmitCount <= ndpContext->dupAddrDetectTransmits)
                  {
                     //Send a multicast Neighbor Solicitation message
                     ndpSendNeighborSol(interface, &entry->addr, TRUE);

                     //Set timeout value
                     entry->dadTimeout += interface->ndpContext.retransTimer;
 ...
 * @param[out] newNextHop Next-hop address to be used
 * @return Error code
 **/
