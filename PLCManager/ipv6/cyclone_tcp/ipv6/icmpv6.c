   ...
   const NetBuffer *request, size_t requestOffset)
{
   ...
   {
      //If support for multicast Echo Request messages has been explicitely
   ...
   uint8_t code, uint32_t parameter, const NetBuffer *ipPacket)
{
   ...
   //Sanity check
   if(ipHeader == NULL) return ERROR_FAILURE;
   ...
   //Failed to allocate memory?
   if(icmpMessage == NULL) return ERROR_OUT_OF_MEMORY;
