...
error_t ndpResolve(NetInterface *interface, const Ipv6Addr *ipAddr, MacAddr *macAddr)
{
   ...
   else
   {
      ...
      {
         //Failed to create Neighbore Cache entry...
...
void ndpTick(NetInterface *interface)
{
   ...
   //Point to the NDP context
   ndpContext = &interface->ndpContext;
   ...
   {
      //Check whether a link-local address has been assigned to the interface
      if(interface->ipv6Context.addrList[0].state == IPV6_ADDR_STATE_PREFERRED)
      {
         //The host should transmit up to MAX_RTR_SOLICITATIONS Router
         //Solicitation messages
         if(ndpContext->rtrSolicitationCount == 0)
         {
            //Set time stamp
            ndpContext->timestamp = time;

            //Check whether the host has already performed Duplicate Address
            //Detection for the link-local address
            if(ndpContext->dupAddrDetectTransmits > 0)
            {
            ...
            else
            {
               ...
               ndpContext->timeout = netGetRandRange(ndpContext->minRtrSolicitationDelay,
                  ndpContext->maxRtrSolicitationDelay);
            }

            //Prepare to send the first Router Solicitation message
            ndpContext->rtrSolicitationCount = 1;
         }
         else if(ndpContext->rtrSolicitationCount <= ndpContext->maxRtrSolicitations)
         {
            //Once the host sends a Router Solicitation, and receives a valid
            //Router Advertisement with a non-zero Router Lifetime, the host must
            //desist from sending additional solicitations on that interface
            if(!ndpContext->rtrAdvReceived)
            {
               //Check current time
               if(timeCompare(time, ndpContext->timestamp + ndpContext->timeout) >= 0)
               {
                  ...
                  ndpContext->timestamp = time;
                  //Set timeout value
                  ndpContext->timeout = ndpContext->rtrSolicitationInterval;
                  //Increment retransmission counter
                  ndpContext->rtrSolicitationCount++;
               }
            }
         }
      }
   ...
   const NetBuffer *buffer, size_t offset, uint8_t hopLimit)
{
   ...
   {
      //In all cases, the Router Soliciation retransmissions are terminated
   ...
   if(!entry)
   {
      ...
      if(linkLayerAddrOption)
      {
         ...
         if(entry)
         {
            ...
            //The neighbor is assumed to be a router
            entry->isRouter = TRUE;
            //Save current time
            entry->timestamp = osGetSystemTime();
   ...
   const Ipv6Addr *targetIpAddr, bool_t multicast)
{
   ...
   {
      //Select the most appropriate source address to be used when
      //sending the Neighbor Solicitation message
      error = ipv6SelectSourceAddr(&interface,
   ...
   const Ipv6Addr *targetIpAddr, const Ipv6Addr *destIpAddr)
{
   ...
   message->r = interface->ipv6Context.isRouter ? TRUE : FALSE;
