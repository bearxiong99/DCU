...
static char *puc_status_txt[3] = {"disconnected", "terminal", "switch"};

typedef struct x_info_nodes {
	uint16_t lnid;
	uint8_t puc_eui48[6];
	uint8_t sid;
	uint8_t lsid;
	...
	uint8_t alv_tx_cnt;
	uint8_t alv_time;
	enum mac_state_val state;
} x_info_nodes_t;

static x_info_nodes_t sx_info_nodes[MAX_NUM_NODES_CONNECTED];
...
static uint8_t puc_mac_address[6];
...
static int si_dlmsoctp_app_id;

/* CLI socket descriptors */
static int si_dlmsoctp_link_fd;
static int si_dlmsoctp_data_fd;

static unsigned char suc_dlmsoctp_rcv_buf[MAX_BUFFER_SIZE];

static int spi_conn_fds[SOCKET_MAX_CONN];
static int si_num_conn;
...
static void _send_queued_432_data()
{
	...
	/* build dl_432_buffer... */
	dl_432_buffer_t msg;
	memcpy(msg.dl.buff, x_pending_cfm_messages_list.px_head->data, x_pending_cfm_messages_list.px_head->us_length);
	msg.dl.lsap = x_pending_cfm_messages_list.px_head->us_lsap;

	//send data
	ifacePrime_select_api(si_dlmsoctp_app_id);
	prime_cl_432_dl_data_request(1, x_pending_cfm_messages_list.px_head->us_lsap,
                                 ...
                                 uint16_t us_dst_address, uint8_t *puc_mac)
{
	...
	if (x_list_nodes_connected[us_dst_address].autoclose_enabled == FALSE) {
		if ((si_dlmsoctp_data_fd > 0) && (i_enable_notifications)) {
			...
			write(si_dlmsoctp_data_fd, puc_dlms_msg, uc_length);
		...
		uint16_t src_address, uint8_t *puc_data, uint16_t uc_lsdu_len, uint8_t uc_link_class)
{
	...
	if ((si_dlmsoctp_data_fd > 0) && (i_enable_notifications)) {
		...
		if (uc_lsdu_len > 0) {
			...
			/* Send notification to Concentrator */
			write(si_dlmsoctp_data_fd, puc_dlms_msg, us_length);
		}
	}
}

uint8_t _get_mac_father(uint8_t sid, uint8_t *puc_dst) {
	x_info_nodes_t *px_node;
	int i;

	if (sid == 0) {
		/* Return own mac address */
		memcpy(puc_dst, puc_mac_address, sizeof(puc_mac_address));
		return 1;
	}

	px_node = sx_info_nodes;
	for (i=0; i < MAX_NUM_NODES_CONNECTED; i++, px_node++) {
		if(px_node->lsid == sid) {
			memcpy(puc_dst, px_node->puc_eui48, sizeof(px_node->puc_eui48));
			return 1;
		}
	}
...
/**
 * \brief Event handler from network event
 * Los datos que contendrá cada registro de los ficheros son los siguientes:
o	Fecha y hora en formato: “yyyy/mm/dd hh:mm:ss”
o	MAC Nodo
o	MAC Nodo Padre (Switch o Nodo Base)
o	Estado
o	Calidad (porcentaje de éxito de los Keep Alive)
o	MAC Nodo Base
 *
 */
void _bmng_network_event_msg_cb(bmng_net_event_t *px_net_event)
{
	...
	int num_char;
	int coverage;
	uint8_t puc_mac_fth[6];

	csv_fd = open("/var/DC/topology.csv", O_RDWR|O_CREAT|O_APPEND, S_IROTH|S_IWOTH);
	...
	if (csv_fd != -1) {
		...
		px_node = &sx_info_nodes[px_net_event->lnid];

		/* Update info from node */
		px_node->lnid = px_net_event->lnid;
		px_node->lsid = px_net_event->lsid;
		px_node->sid = px_net_event->sid;
		memcpy(px_node->puc_eui48, px_net_event->mac, sizeof(px_node->puc_eui48));
		px_node->alv_rx_cnt = px_net_event->alvRxcnt;
		px_node->alv_tx_cnt = px_net_event->alvTxcnt;
		px_node->alv_time = px_net_event->alvTime;
		coverage = (px_node->alv_rx_cnt / px_node->alv_tx_cnt) * 100;

		/* Get mac address from father */
		_get_mac_father(px_node->sid, puc_mac_fth);

		switch(px_net_event->net_event) {
		case BMNG_NET_EVENT_REGISTER:
			px_node->state = TERMINAL;
			break;

		case BMNG_NET_EVENT_UNREGISTER:
			px_node->state = DISCONNECTED;
			break;

		case BMNG_NET_EVENT_PROMOTE:
			px_node->state = SWITCH;
			break;
		...
		/* 2013/02/05 10:28:56;40:40:22:29:14:73;40:40:22:01:9f:f0;disconnected;0;40:40:22:01:9f:f0 */
		puc_buff_ptr = puc_buff;

		num_char = sprintf(puc_buff_ptr, "%d/%d/%d %02d:%02d:%02d;", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
				tm.tm_hour, tm.tm_min, tm.tm_sec);
		puc_buff_ptr += num_char;

		num_char = sprintf(puc_buff_ptr, "%02x:%02x:%02x:%02x:%02x:%02x;",
				px_net_event->mac[0], px_net_event->mac[1], px_net_event->mac[2],
				px_net_event->mac[3], px_net_event->mac[4], px_net_event->mac[5]);
		puc_buff_ptr += num_char;

		num_char = sprintf(puc_buff_ptr, "%02x:%02x:%02x:%02x:%02x:%02x;",
				puc_mac_fth[0], puc_mac_fth[1], puc_mac_fth[2],
				puc_mac_fth[3], puc_mac_fth[4], puc_mac_fth[5]);
		puc_buff_ptr += num_char;

		num_char = sprintf(puc_buff_ptr, "%s;", puc_status_txt[px_node->state]);
		puc_buff_ptr += num_char;

		num_char = sprintf(puc_buff_ptr, "%d;", coverage);
		puc_buff_ptr += num_char;

		num_char = sprintf(puc_buff_ptr, "%02x:%02x:%02x:%02x:%02x:%02x\n",
				puc_mac_address[0], puc_mac_address[1], puc_mac_address[2],
				puc_mac_address[3], puc_mac_address[4], puc_mac_address[5]);
		puc_buff_ptr += num_char;

		write(csv_fd, puc_buff, (puc_buff_ptr - puc_buff));
		close(csv_fd);
	}
}
    ...
    if (*ptr != MNGP_PRIME_LISTRSP) {
        ...
        if (us_pib_attrib == PIB_ATML_432_NODE_LIST) {
			...
			if ((uc_records == 0) || (uc_records < MNGP_PRIME_ENHANCED_LIST_MAX_RECORDS)) // Last element -> push
			{
				...
				/* End query */
				if ((puc_mac_address[0] == 0) && (puc_mac_address[1] == 0) && (puc_mac_address[2] == 0) &&
					(puc_mac_address[3] == 0) && (puc_mac_address[4] == 0) && (puc_mac_address[5] == 0)) {
					/* Get MAC address */
					mngLay_NewMsg(MNGP_PRIME_GETQRY);
					mngLay_AddGetPibQuery(PIB_MAC_EUI_48, 0);
					mngLay_SendMsg(si_dlmsoctp_app_id);
				}
...
int dlmsotcp_init(int _app_id)
{
	...
	si_dlmsoctp_app_id = _app_id;
	si_dlmsoctp_data_fd = -1;
	si_dlmsoctp_link_fd = -1;
	...
	prime_cl_432_set_callbacks(si_dlmsoctp_app_id, &st_cl_432_cbs);
	
	/* PRIME Mng : Use Enhanced to query 8250 list (432 connected nodes) */
	mngLay_SetRspCallback(_app_id, _dlmsotcp_mngp_rsp_cb);
	mngLay_NewMsg(MNGP_PRIME_EN_PIBQRY);
	...
	mngLay_SendMsg(_app_id);
	
	memset(puc_mac_address, 0,  sizeof(puc_mac_address));
    ...
    if ((usSource == 1) && (usDest == 0) && (usDlmsLen >= 1)) {
        ...
        if (px_msg) {
            ...
            if (x_pending_cfm_messages_list.us_count == 0) {
            	...
            	ifacePrime_select_api(si_dlmsoctp_app_id);
            	prime_cl_432_dl_data_request(1, dlms_432_msg.dl.lsap, usDest, &dlms_432_msg, usDlmsLen, 0);
            }
...
void dlmsotcp_callback(socket_ev_info_t *_ev_info)
{
	if (_ev_info->i_socket_fd >= 0) {
		...
		if (_ev_info->i_event_type == SOCKET_EV_LINK_TYPE) {
			/* Manage LINK */
			si_dlmsoctp_link_fd = _ev_info->i_socket_fd;
			socket_accept_conn(_ev_info);
			/* Maintain only one connection for PLC_MNG_DLMSoTCP_APP_ID */
			si_num_conn = socket_get_connections_fds(si_dlmsoctp_app_id, spi_conn_fds);
			if (si_num_conn > 1) {
				/* Close the last one */
				socket_dettach_connection(si_dlmsoctp_app_id, spi_conn_fds[1]);
			}
		} else if (_ev_info->i_event_type == SOCKET_EV_DATA_TYPE) {
			/* Receive DATA */
			ssize_t i_bytes;
			si_dlmsoctp_data_fd = _ev_info->i_socket_fd;
			/* Read data from Socket */
			i_bytes = read(_ev_info->i_socket_fd, suc_dlmsoctp_rcv_buf, MAX_BUFFER_SIZE);
			if (i_bytes > 0) {
				dlmsotcp_RxProcess(suc_dlmsoctp_rcv_buf, i_bytes);
			} else {
				socket_check_connection(_ev_info->i_app_id, _ev_info->i_socket_fd);
				/* Check connections to close 432 */
				si_num_conn = socket_get_connections_fds(si_dlmsoctp_app_id, spi_conn_fds);
				if (si_num_conn == 0) {
					si_dlmsoctp_data_fd = -1;
					dlmsotcp_close_432();
