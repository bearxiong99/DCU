...
int xml_rep_open(void)
{
	...
	remove("/tmp/nodelist.xml");

	/* Create XML file with the list of connected nodes */
	xml_fd = open("/tmp/nodelist.xml", O_RDWR|O_CREAT, S_IROTH|S_IWOTH|S_IXOTH);
...
void xml_rep_close(int xml_fd)
{
	...
	remove("/home/DCWS/nodelist.xml");

	xml_fd1 = open("/tmp/nodelist.xml", O_RDONLY);
	xml_fd2 = open("/home/DCWS/public/nodelist.xml", O_RDWR|O_CREAT, S_IROTH|S_IWOTH|S_IXOTH);
