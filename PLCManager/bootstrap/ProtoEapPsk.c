...
void EAP_PSK_Initialize(const struct TEapPskKey *pKey, struct TEapPskContext *pPskContext)
{
  ...
  uint8_t au8Block[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00 };
  uint8_t au8Res[16] =
  ...
  aes_encrypt_key128(pKey->m_au8Value, aesCtx);
  ...
  // generate AK
  aes_encrypt(au8Res, pPskContext->m_Ak.m_au8Value, aesCtx);
  ...
  // generate KDK
  aes_encrypt(au8Res, pPskContext->m_Kdk.m_au8Value, aesCtx);
}
...
void EAP_PSK_InitializeTEK(const struct TEapPskRand *pRandP, struct TEapPskContext *pPskContext)
{
  ...
  // initialize the AES context
  aes_encrypt_key128(pPskContext->m_Kdk.m_au8Value, aesCtx);

  aes_encrypt(pRandP->m_au8Value, au8Res, aesCtx);

  // xor with c1 = "1"
  au8Res[15] ^= 0x01;

  // generate TEK
  aes_encrypt(au8Res, pPskContext->m_Tek.m_au8Value, aesCtx);
}
  ...
  const struct TEapPskNetworkAccessIdentifier *pIdP, uint16_t u16MemoryBufferLength, uint8_t *pMemoryBuffer)
{
  ...
  if (u16MemoryBufferLength >= 62) {
    ...
    cmac_init(pPskContext->m_Ak.m_au8Value, sizeof(pPskContext->m_Ak.m_au8Value), ctx);
    cmac_data(au8Seed, u16SeedUsedSize, ctx);
    cmac_end(au8MacP, ctx);
  ...
  uint8_t *pu8PChannelResult, uint16_t *pu16PChannelDataLength, uint8_t **pPChannelData)
{
  ...
  if (u16MessageLength >= 59) {
    ...
    cmac_init(pPskContext->m_Ak.m_au8Value, sizeof(pPskContext->m_Ak.m_au8Value), cmacCtx);
    cmac_data(au8Seed, u16SeedUsedSize, cmacCtx);
    cmac_end(au8MacS, cmacCtx);
  ...
  uint8_t *pPChannelData, uint16_t u16MemoryBufferLength, uint8_t *pMemoryBuffer)
{
  ...
  if (u16MemoryBufferLength >= 43 + u16PChannelDataLength) {
    ...
    au8Nonce[12] = pMemoryBuffer[u16Ret] = (uint8_t)(u32Nonce >> 24);
    au8Nonce[13] = pMemoryBuffer[u16Ret + 1] = (uint8_t)(u32Nonce >> 16);
    au8Nonce[14] = pMemoryBuffer[u16Ret + 2] = (uint8_t)(u32Nonce >> 8);
    au8Nonce[15] = pMemoryBuffer[u16Ret + 3] = (uint8_t)(u32Nonce);
    u16Ret += 4;
...
)
{
    ...
    //enum EMacStatus x_status;
    struct TMacPibValue x_pib_value;
    ...
    if (u16MessageLength >= uc_min_msg_length) // EAP header already removed
    {
        ...
        cmac_init(pPskContext->m_Ak.m_au8Value, sizeof(pPskContext->m_Ak.m_au8Value), ctx);
        cmac_data(au8Seed, au8Seed_size, ctx);
        cmac_end(au8ExpectedMacP, ctx);
